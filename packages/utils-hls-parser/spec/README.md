...
**Package**: `@mtngtools/utils-hls-parser`
...
4.  **Output**: Return structured, typed objects (defined in [`@mtngtools/utils-hls-types`](../../utils-hls-types/spec/README.md)).
...
**Status**: Deferred - Current implementation focuses on VOD (Video On Demand) transfers. Live stream support would require additional orchestration logic in `utils-hls-core`.
...
- **Note**: `MediaGroup` type is defined in `@mtngtools/utils-hls-types` but not fully integrated into variant parsing
